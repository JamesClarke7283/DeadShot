...
These counts describe the authored export. `tools/deadshot_enrich_maps.py` appends architectural detail on top of it — parapets above each roof edge, a base plinth at each wall foot, window and door frames with glazing on every facade, a parapet drip line and a downpipe — plus detail on the authored props: wheel arches, bumpers and lamps on each vehicle, cabin glazing, lattice bracing and an operator cab on each crane, pitched canopies and valances over the market stalls, a curved reflector and feed horn on each radar, and end posts with reflective banding on the barriers.

The pass is purely additive and idempotent: it never edits or removes an authored node, and re-running it replaces its own detail rather than stacking it. Collision boxes, spawn pads and the navigation graph are therefore untouched, and `verify_maps.gd` asserts the authored counts alongside the detail slots on every map.

Prop detail is derived per *part*, not per instance. A prop is authored as several boxes whose world positions overlap heavily — four cars parked in a row stand about 1.8 m apart while the parts of one car sit within 1 m of each other — so no proximity threshold separates instances reliably. Keying off each part's own proportions is exact instead: a car's chassis is its wide, low part and its cabin the narrow, raised one, wherever they stand. The authored wheels are kept as they are and only the surrounding detail is added, which is why the vehicles keep their original silhouette.

The authored palette is authoritative for the keys it defines, and `authoredLighting` packs its colours as `0xRRGGBB` integers while the per-map `environment` block uses Three's linear arrays. `MapWorld._build_environment()` converts the packed values and merges the authored palette over the environment block rather than replacing it, so the authored `fogColor` and `background` survive. Replacing the block outright discarded them and fell back to a generic cool grey fog, which is why an earlier build of the ported desert rendered with a grey haze instead of the authored warm one.
